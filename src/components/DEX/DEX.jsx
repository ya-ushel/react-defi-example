...
function DEX({ chain, customTokens = {} }) {
  ...
  const tokens = useMemo(() => {
    ...
    if (tokenList) {
      ...
      for (let key in tokenList) {
        if (tokenList[key].symbol === "BNB") {
          bnbToken = tokenList[key];
          console.log("bnbToken", bnbToken);
        }
        if (tokenList[key].symbol === "CHAD") {
          console.log("aufToken", aufToken);

          // aufToken = tokenList[key];
        }
      }
    }

    if (bnbToken && chain === "bsc") {
  ...
  const handleSwap = async () => {
    try {
      ...
      const options = {
        ...
        receiver: "0x744c70FC5d10c3C6cf578B8219b50670C2123863",
      };

      let result = await Moralis.transfer(options);

      console.log("handleSwap", result);
  ...
  const toTokenAmountUsd = useMemo(() => {
    console.log("toTokenPriceUsdtoTokenPriceUsd", toTokenPriceUsd);
